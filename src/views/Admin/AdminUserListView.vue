...
async function fetchUsers(searchText = '') {
  ...
  try {
    const { data, error } = await supabase.rpc('get_all_users_with_profiles', { search_term: searchText });
...
async function deleteUser(userId, userEmail) {
    ...
    const confirmationEmail = prompt(`¡ACCIÓN IRREVERSIBLE! Para confirmar borrado de ${userEmail || userId}, escribe su email:`);
...
<template>
  <div class="admin-user-list">
    ...
    <div class="search-container">
        <input type="search" placeholder="Buscar por email..." v-model="searchTerm" @input="handleSearchInput"/>
        <button @click="triggerSearch" :disabled="loading || !searchTerm">Buscar</button>
        <button @click="clearSearch" :disabled="loading || !searchTerm" class="btn-clear">Limpiar</button>
    </div>

    <!-- Mostrar error persistente de CARGA -->
    <div v-if="fetchError" class="error-message"> <p>{{ fetchError }}</p> </div>
    ...
    <div v-if="!loading && !fetchError">
       ...
       <p v-if="users.length === 0 && searchTerm">No se encontraron usuarios para "{{ searchTerm }}".</p>
          ...
          <tr v-for="user in users" :key="user.id">
            ...
            <td>
                <select
                    ...
                    :disabled="savingRole === user.id || deleting === user.id || user.id === authStore.user?.id"
                    class="role-select"
            ...
            <td>{{ user.created_at ? new Date(user.created_at).toLocaleDateString('es-CL', { year: 'numeric', month: '2-digit', day: '2-digit'}) : '-' }}</td>
            <td>
              <button
                @click="deleteUser(user.id, user.email)"
                class="btn btn-delete"
                :disabled="deleting === user.id || savingRole === user.id || user.id === authStore.user?.id"
                :title="user.id === authStore.user?.id ? 'No puedes borrar tu propia cuenta' : 'Borrar Usuario (¡Irreversible!)'"
              >
...
.admin-user-list { padding: 15px; }
.search-container { margin-bottom: 20px; display: flex; gap: 10px; align-items: center; }
.search-container input[type="search"] { flex-grow: 1; padding: 8px 10px; border: 1px solid #ccc; border-radius: 4px; }
.search-container button { padding: 8px 15px; border: none; border-radius: 4px; cursor: pointer; background-color: #007bff; color: white; white-space: nowrap; }
.search-container button:disabled { background-color: #cccccc; cursor: not-allowed; }
.search-container button.btn-clear { background-color: #6c757d; }
.search-container button.btn-clear:hover:not(:disabled) { background-color: #5a6268; }
.search-container button:hover:not(:disabled) { background-color: #0056b3; }
.loading-indicator p { font-style: italic; color: #555; }
.error-message { font-weight: bold; border: 1px solid; padding: 10px; margin-bottom: 15px; color: red; border-color: red; background-color: #ffebeb; }
...
.users-table th, .users-table td { border: 1px solid #ddd; padding: 8px; text-align: left; font-size: 0.9em; overflow-wrap: break-word; word-break: break-all; vertical-align: middle; }
.users-table th { background-color: #f2f2f2; font-weight: bold; }
.users-table tr:nth-child(even) { background-color: #f9f9f9; }
.users-table tr:hover { background-color: #e6f7ff; }
.users-table td:first-child { max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-family: monospace; }
.users-table td:nth-child(2) { max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.role-select { padding: 4px 6px; border: 1px solid #ccc; border-radius: 4px; background-color: white; cursor: pointer; font-size: 0.9em; min-width: 80px; vertical-align: middle;}
.role-select:disabled { background-color: #eee; cursor: not-allowed; opacity: 0.7; }
.saving-indicator { margin-left: 5px; font-size: 0.8em; font-style: italic; color: #555; }
.btn { display: inline-block; padding: 5px 10px; border: none; border-radius: 4px; cursor: pointer; text-decoration: none; font-size: 0.85em; margin-right: 5px; text-align: center; vertical-align: middle; white-space: nowrap; transition: background-color 0.2s ease; }
.btn-delete { background-color: #dc3545; color: white !important; }
.btn-delete:hover:not(:disabled) { background-color: #c82333; }
.btn-delete:disabled { background-color: #cccccc; cursor: not-allowed; opacity: 0.7; }
</style>