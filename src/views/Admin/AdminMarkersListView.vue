...
async function fetchMarkers() {
  ...
  try {
    const { data, error } = await supabase
      .from('markers')
      .select(
        `id, created_at, name, mind_file_name, is_public, user_id, marker_contents(contents(id, name, type))`,
      )
    ...
    if (error) {
      ...
      markers.value = (data || []).map((marker) => ({
        ...marker,
        associated_contents: marker.marker_contents
          .map((mc) => mc.contents)
          .filter((c) => c !== null),
      }))
    }
...
async function deleteMarker(markerId) {
  ...
  if (!confirm(`¿Seguro que quieres borrar el marcador ID: ${markerId}?`)) {
    return
  }
  deleting.value = markerId
  errorMsg.value = ''
  try {
      ...
      <table v-else class="markers-table">
        ...
        <tbody>
          <tr v-for="marker in markers" :key="marker.id">
            ...
            <td
              class="associated-contents-cell"
              :title="formatAssociatedContents(marker.associated_contents)"
            >
              {{ formatAssociatedContents(marker.associated_contents) }}
            </td>
            <td>{{ marker.is_public ? 'Sí' : 'No' }}</td>
            <td>
              {{ marker.created_at ? new Date(marker.created_at).toLocaleString('es-CL') : '-' }}
            </td>
            <td>
              <router-link
