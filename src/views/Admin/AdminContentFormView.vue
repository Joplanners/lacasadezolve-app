...
const formData = ref({
    id: null, name: '', type: '', content_url: '', user_id: null, is_public: true
});
...
const handleFileChange = (event) => {
    ...
    if (files && files.length > 0) {
        ...
        console.log("Archivos seleccionados:", selectedFiles.value.map(f => f.name));
    } else {
        ...
        if (!formData.value.is_public && formData.value.user_id) {
            ...
            if (ownerError) {
                ...
                assignedUser.value = {
                    ...
                    name: `${ownerData.user_first_name || ''} ${ownerData.user_last_name || ''}`.trim() || ownerData.user_email
                };
                userSearchTerm.value = assignedUser.value.email || assignedUser.value.id;
                console.log('Dueño inicial cargado:', assignedUser.value);
            } else {
                 console.warn(`Búsqueda de dueño por ID ${formData.value.user_id} devolvió ${ownerResults ? ownerResults.length : 0} resultados.`);
...
const searchUsers = async () => {
    if (assignedUser.value && userSearchTerm.value === (assignedUser.value.email || assignedUser.value.id)) {
        userSearchResults.value = []; showUserDropdown.value = false; return;
    }
    if (!userSearchTerm.value || userSearchTerm.value.length < 2) {
        ...
        if (assignedUser.value) {
            assignedUser.value = null; formData.value.user_id = null;
        } return;
    }
    ...
    try {
        ...
        userSearchResults.value = []; showUserDropdown.value = true;
...
async function saveContent() {
    ...
    if (props.isEditMode) {
        ...
        let finalUserId = formData.value.is_public ? null : (assignedUser.value?.id || formData.value.user_id || authStore.user?.id || null);
        ...
        try {
            ...
            if (singleFileToUpload) {
                ...
                if (!response.ok) {
                    ...
                    try { const errJson = await response.json(); workerErrorMsg += `: ${errJson.error || response.statusText}`; } catch (e) { /* ignore */ }
            ...
            console.log(`Actualizando en DB: ID=${formData.value.id}, URL=${finalContentUrl}, UserID=${finalUserId}, Public=${formData.value.is_public}`);
            ...
            const { error: dbError } = await supabase.from('contents').update(contentDataToUpdate).eq('id', formData.value.id);
...
onMounted(() => {
    if (props.isEditMode) {
        ...
        if (contentId) {
            ...
            errorMsg.value = idError; toast.error(idError);
        }
    }
...
<template>
    <div class="admin-content-form">
        ...
        <div v-if="errorMsg && !saving || errorMsg && saving && uploadProgress.errors > 0" class="error-message"><p>{{ errorMsg }}</p></div>
        <!-- Mensaje de progreso durante la subida -->
        <div v-if="saving && uploadProgress.total > 0 && uploadProgress.errors === 0" class="saving-message">
        ...
        <form v-if="!loadingData" @submit.prevent="saveContent">
            ...
            <div v-if="!props.isEditMode" class="info-box">
                <p>El <strong>nombre</strong> y el <strong>tipo</strong> de cada contenido se tomarán automáticamente del archivo subido.</p>
            </div>
            ...
            <div class="form-group">
                ...
                <!-- Mensajes de ayuda condicionales -->
                 <small v-if="!props.isEditMode">Selecciona uno o más archivos (imagen, video, texto).</small>
                <small v-if="props.isEditMode && formData.content_url">URL actual: {{ formData.content_url }}. Selecciona un archivo para reemplazarlo.</small>
                <small v-if="props.isEditMode && !formData.content_url">No hay archivo asociado. Selecciona uno.</small>
                ...
                <ul v-if="selectedFiles.length > 0" class="file-list">
                     <li v-for="(file, index) in selectedFiles" :key="index" class="file-info">
                         {{ file.name }} ({{ (file.size / 1024 / 1024).toFixed(2) }} MB) - Tipo: {{ getFileType(file.type) }}
                     </li>
                 </ul>
            </div>
            ...
            <div class="form-group checkbox-group">
                 ...
                 <small>Si está marcado, todos lo podrán ver. Si no, asigna un usuario o se asignará a tu cuenta.</small>
            </div>
            ...
            <div class="form-group" id="user-search-container" v-if="!formData.is_public">
                ...
                <input type="search" id="userSearch" v-model="userSearchTerm" @input="debouncedSearchUsers" @focus="showUserDropdown = true" placeholder="Buscar email o nombre..." autocomplete="off">
            ...
            <div class="form-actions">
                <button type="submit" class="btn btn-save" :disabled="saving || (!props.isEditMode && selectedFiles.length === 0 && !formData.id) || (props.isEditMode && !formData.id)">
                    {{ saving ? (uploadProgress.total > 0 ? `Procesando ${uploadProgress.current}/${uploadProgress.total}...` : 'Guardando...') : (props.isEditMode ? 'Guardar Cambios' : 'Subir y Guardar') }}
                </button>
                <router-link :to="{ name: 'admin-contents' }" class="btn btn-cancel" :class="{ disabled: saving }">Cancelar</router-link>
            </div>
        </form>
    </div>
</template>
...
<style scoped>
/* Estilos existentes... */
.admin-content-form { max-width: 600px; margin: 20px auto; padding: 25px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9; }
...
.form-group input[type="text"], .form-group input[type="url"], .form-group input[type="file"], .form-group input[type="search"], .form-group select { width: 100%; padding: 8px 10px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box; }
